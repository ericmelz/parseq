...
describe('dotify', function() {
  describe('escapes', function() {
    ['<', '>', '|', '{', '}'].forEach(function(specialChar) {
      it('escapes `' + specialChar + '` for the Mrecord shape', function() {
        ...
        assert.equal(dotify(g).node(1).label, 'before\\' + specialChar + 'after\\l|{@0|+0}');
      });
    });
  });

  describe('style', function() {
    it('sets the orientation to LR', function() {
      var g = new CDigraph();
      var d = dotify(g);
      assert.equal(d.graph().rankdir, 'LR');
    });
    ...
    it('styles nodes', function() {
      ...
      var d = dotify(g);
      assert.equal(d.node(1).label, 'foo task\\l|{@0|+500}');
    ...
    it('styles composed tasks', function() {
      ...
      g.addNode('child', {
        startNanos: 10 * 1000 * 1000,
        endNanos:  450 * 1000 * 1000
      });
      g.parent('child', 'parent');

      var d = dotify(g);
      assert.propertyVal(d.node('cluster_parent'), 'label', 'parent task (@0, +500)');
  ...
  describe('handles edges', function() {
    ...
    it('creates edges with 2 level nested task', function() {
      ...
      assertEdgesFrom(d, 't1',            { source_parent: 'solid', t3: 'invis' });
