...
describe('d3Treeify', function() {
  ...
  it('adds an elapsed property derived from startNanos and endNanos', function() {
    var g = new CDigraph();
    ...
    var root = d3Treeify(g);
    assert.deepPropertyVal(root, 'children[0].id', 1);
    assert.deepPropertyVal(root, 'children[0].elapsed', 75);
  });
  ...
  it('sets start time relative to smallest start time in the graph', function() {
    ...
    var root = d3Treeify(g);
    assert.deepPropertyVal(root, 'children[0].start', 0);
    assert.deepPropertyVal(root, 'children[0].elapsed', 450);
    assert.deepPropertyVal(root, 'children[0].children[0].start', 50);
    assert.deepPropertyVal(root, 'children[0].children[0].elapsed', 200);
  });
