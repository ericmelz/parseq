...
var floydWarshall = require('graphlib').alg.floydWarshall;
...
function rewriteNodes(g, parents) {
  ...
  g.eachNode(function(u, value) {
    var start   = '@' + Math.round((value.startNanos - minStartNanos) / (1000 * 1000)),
        elapsed = '+' + Math.round((value.endNanos - value.startNanos) / (1000 * 1000));

    if (parents.has(u)) {
      g.node(u, {
        label: value.name + ' (' + start + ', ' + elapsed + ')',
        labeljust: 'l',
        style:     'dashed',
        color:     '#aaaaaa',
      ...
      g.node(u, {
        label: escapeLabel(value.name) + '\\l|{' + start + '|' + elapsed + '}',
        shape: 'Mrecord',
        style: 'filled',
        fillcolor: fillcolor(value.resultType)
      });
...
function addSourceSinkEdges(g, potentialParents, parents, originalG) {
  var paths = floydWarshall(originalG),
