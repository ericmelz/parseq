...
function redraw(svg, vis, data, ancestors, transitionTime, rootCoords) {
  ...
  var x = d3.scale.linear()
    .domain([0, d3.max(nodes, function(d) { return d.start + d.elapsed; })])
    .range([0, WIDTH]);
  ...
  barsEnter
    ...
    .append('rect')
      .attr('rx', 3)
      .attr('width', function(d) { return Math.max(x(d.elapsed), MIN_BAR_WIDTH); })
  ...
  function toggleCollapse(d, i) {
    ...
    redraw(svg, vis, data, ancestors, TRANSITION_TIME, { x: x(d.start), y: y(i) });
  }
